...
def router_check(session, intfs, sh_routing):
    """Checks if device is doing inter-vlan routing and on what VLANs or if device is active router"""
    ip_vlan_count = 0
    ...
    for intf in intfs:
        if intf['ipaddr'] != 'unassigned' and (intf['status'] and intf['proto']) == 'up' \
                and 'Vlan' in intf['intf']:
            if nxos and 'up' in intf['link']:
                ip_vlan_count += 1
            elif not nxos:
                ip_vlan_count += 1
        if intf['ipaddr'] != 'unassigned' and (intf['status'] and intf['proto']) == 'up' \
